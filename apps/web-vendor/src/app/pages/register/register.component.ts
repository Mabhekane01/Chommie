    ...
    <div class="min-h-screen bg-[#FAF3E1] flex flex-col items-center pt-12 px-4 text-neutral-charcoal font-body">
      ...
      <div class="w-full max-w-[350px] bg-white border border-neutral-300 rounded-lg p-6 shadow-sm">
        ...
        <ng-container *ngIf="step() === 'details'">
            <h1 class="text-2xl font-medium text-[#222222] mb-4">Create Seller Account</h1>
            ...
            <div class="text-[11px] text-neutral-600 leading-tight pt-2 border-t border-neutral-100">
                By creating an account, you agree to Chommie's <a href="#" class="text-primary hover:underline">Seller Agreement</a> and <a href="#" class="text-primary hover:underline">Privacy Notice</a>.
            </div>

            <div class="pt-4 mt-4 border-t border-neutral-200 text-xs text-center">
                <span class="text-[#222222]">Already have a seller account? </span>
                <a routerLink="/login" class="text-primary hover:underline group font-bold">
                    Sign-In <svg class="inline w-3 h-3 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7" /></svg>
                </a>
            </div>
            </form>
        </ng-container>
        ...
        <ng-container *ngIf="step() === 'otp'">
            ...
            <p class="text-sm text-neutral-700 mb-6 leading-relaxed">
                To verify your seller identity, we've sent a code to <span class="font-bold">{{ email }}</span> <a (click)="step.set('details')" class="text-xs text-primary hover:underline cursor-pointer">(Change)</a>
            </p>
            ...
            <form (ngSubmit)="onVerifyAndRegister()" class="space-y-4">
                ...
                <div class="pt-2">
                    <button type="submit" [disabled]="loading()" class="w-full btn-primary py-1.5 rounded-[3px] text-xs font-normal">
                        {{ loading() ? 'Verifying...' : 'Create your Seller account' }}
                    </button>
                </div>
