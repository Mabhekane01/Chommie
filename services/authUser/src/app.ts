...
import { withAuth } from '@packages/types/express';

// Import controllers
import { initiateLogin } from './controllers/login.js';
...
// Import controllers
import { logoutSession } from './controllers/logoutSession.js';

// Import controllers
import { logoutAllSessions } from './controllers/logoutAllSessions.js';

import { getCurrentSession } from './controllers/getCurrentSession.js';
...
app.post('/auth/login', initiateLogin);
app.post('/auth/verify-otp', verifyOtpController);
app.post('/auth/refresh', refresh);

// Protected session management routes
app.get('/auth/sessions', withAuth(getUserSessions));
app.get(
  '/auth/sessions/current',

  withAuth(getCurrentSession)
);
app.get('/auth/sessions/history', withAuth(getLoginHistory));
app.delete(
  '/auth/sessions/:sessionId',

  withAuth(logoutSession)
);
app.delete('/auth/sessions', withAuth(logoutAllSessions));
