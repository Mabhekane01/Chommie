...
import { Concierge } from '../discovery/concierge';

@Injectable()
export class RecommendationsService {
  /** Grounded LLM concierge; no-op (heuristic fallback) when ANTHROPIC_API_KEY is unset. */
  private readonly concierge = new Concierge();
  ...
  async chat(data: { userId?: string; query: string }) {
    // 0. Grounded LLM concierge when configured (docs/discovery-algorithm.md §5).
    if (this.concierge.enabled) {
      const reply = await this.concierge.chat(data.query, data.userId);
      if (reply) return reply;
    }

    const query = data.query.toLowerCase();

    // 1. Heuristic fallback (used when no ANTHROPIC_API_KEY / LLM unavailable)
