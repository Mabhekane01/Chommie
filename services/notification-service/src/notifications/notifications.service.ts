...
export class NotificationsService {
  ...
  constructor(
    ...
    // Secrets come from env only — never hardcode keys or sender domains.
    const apiKey = this.configService.get<string>('RESEND_API_KEY');
    this.fromEmail = this.configService.get<string>('RESEND_FROM') || 'no-reply@chommie.co.za';
  ...
  async sendEmail(to: string, subject: string, body: string): Promise<boolean> {
    if (!this.resend) {
      this.logger.warn(`Email skipped (RESEND_API_KEY not set): [${subject}] → ${to}`);
      return false;
    }
